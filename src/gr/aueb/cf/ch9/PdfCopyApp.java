...
public class PdfCopyApp {
    ...
    public static void main(String[] args) {
        ...
        try (FileInputStream fis = new FileInputStream("C:/temp/dummy.pdf");
            ...
            while ((b = fis.read()) != -1) {
                fos.write(b);
                counter++;              //gia na metraw ta sunolika byte (read read diavazei q byte thn fora)
            }
            end = System.currentTimeMillis();
            elapsedTime = (end - start) / 1000.0;   //vazontas .0 sto 1000 kanw automata olh thn parastash double

            System.out.printf("Το αρχείο με μέγεθος %.1fKB (%d bytes) αντιγράφηκε επιτυχώς\n", //%f float or double values
                    (counter / 1024.0), counter);               //%.1f float h double me ena dekadiko
